...
- **英語の引用・原文を使う場合は必ず日本語訳を直後に付ける**
- 形式: 英語原文の後に `（日本語訳: 〜）` または改行して括弧内に日本語訳を配置する
- 公式ドキュメント・GitHub・海外ブログからの引用はすべてこのルールを適用する
- 例:
  ```
  "Claude won't create a team without your approval."
  （日本語訳: あなたの承認なしにチームを作ることはありません。）
  ```

### CLAUDE.md・設定ファイルのサンプルコード

- **CLAUDE.md のサンプルコードは日本語で記述する**
- 英語で書かれた CLAUDE.md の例示・事例を記事に掲載する際も、日本語に翻訳して示す
- 例（❌NG）: `"proactively propose creating an agent team"`
- 例（✅OK）: `"積極的にエージェントチームの作成を提案してください"`
- コマンド・コード自体（`git commit` や `uv run` など）は英語のままでよい。あくまで **自然言語の指示文** を日本語にする
...
<!-- IMAGE: eyecatch -->
```
→ **記事本文には配置しない**。eyecatch は WordPress のアイキャッチ画像（Featured Image）として別途設定されるため、`article.html` 内に `<!-- IMAGE: eyecatch -->` プレースホルダーを挿入してはいけない。ただし `image_requests.json` の `"eyecatch"` キーは必ず含めること（画像生成・Featured Image 設定に使用される）。
...
記事の「まとめ」セクション（最後のH2）の**直前**に、以下のプレースホルダーを配置する。
後続のアフィリエイトリンク生成エージェントがこれを読み取って書籍リンクを挿入する。

```html
<!-- AFFILIATE_BOOKS -->
```

→ このプレースホルダーは1記事に1つだけ配置する。配置場所は必ず「まとめ」のH2の直前とする。

## 出力ファイル

指示された `drafts/{slug}/` ディレクトリに以下の3ファイルを保存する。

### 1. article.html

記事HTML全文。画像プレースホルダーとアフィリエイトプレースホルダーを含む。
...
### 3. affiliate_requests.json

アフィリエイトリンク生成エージェントへのリクエスト。記事テーマに関連する書籍の検索キーワードを出力する。

```json
{
  "keywords": ["テーマに関連する書籍の検索キーワード1", "キーワード2"],
  "context": "記事の内容を簡潔に説明する文",
  "max_books": 2
}
```

**キーワード選定のルール**:
- 記事テーマに直接関連する書籍が見つかりそうなキーワードを2〜3個選ぶ
- 「入門」「実践」「初心者」などの修飾語をつけると書籍が見つかりやすい
- 例: テーマ「Claude Code」→ `["AIコーディング 入門 書籍", "Claude AI プログラミング 本"]`
...
- [ ] 40文字を超える文が全体の25%以下
- [ ] `<!-- AFFILIATE_BOOKS -->` プレースホルダーが「まとめ」セクションの直前にある
- [ ] `affiliate_requests.json` が出力されている
